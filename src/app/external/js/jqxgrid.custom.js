...
(function (b) {
	...
	b.extend(b.jqx._jqxGrid.prototype, {
				...
				if (this.width != "auto" && !this._width && !p) {
					...
					if (p) {
						...
						if (l - m < 3 && l != m) {
							...
							if (q == k.columns.records.length - 1) {
								if (m < l) {
									r += (l - m)
								}
							}
		...
		_rendervisualrows : function () {
			...
			// to handle schedule information columns
			// cellsrenderer must have .scheduleDiv with a or label as children, plus a hidden tag with name=rowId
			if (!this.autorowheight) {
				var tempHeights = this.heights;
				var defaultHeight = this.rowsheight;
				var currentHeight;
				if ($(this.element).find("div[role=row]").has(".scheduleDiv").length > 0) {
					
					$(this.element).find("div[role=row]").has(".scheduleDiv").each(function(rowIdx, row) { 
						var totalHeight = 0;
						var rowIndex;
						
						$(row).find(".scheduleDiv").each(function(scheduleDivIdx, scheduleDiv) {
							
							$(scheduleDiv).find("a, label").each(function(elemIdx, element) {
								totalHeight += $(element).height() + 5;
							})

							rowIndex = $(scheduleDiv).find("[name=rowId]").val();
						})

						if (totalHeight > 0) {
							tempHeights[rowIndex] = totalHeight;
							currentHeight = totalHeight;
						} else {
							tempHeights[rowIndex] = defaultHeight;
		...
		_rendercell : function (v, f, k, t, d, r) {
			...
			if (v.editcell && v.editrow == undefined) {
				if ((v.editmode == "selectedrow" || v.editmode == "programmatic") && f.editable && v.editable) {
